					...
					<li class="item-<?= ($x+1) ?> pull-left <?php echo (!isset($model->review_foto[$x])) ? 'no-foto' : '' ?>" data-item="<?= ($x+1) ?>">
						<?php 
							if(isset($model->review_foto[$x]))	{
								echo Html::a(Html::img(Url::home(true) . 'tmp/thumb_' .$model->review_foto[$x]), Url::home(true) . 'tmp/' .$model->review_foto[$x], ['class' => '', 'data-toggle' => 'lightbox', 'data-gallery'=>'examplesimages']);
